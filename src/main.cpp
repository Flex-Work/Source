...
#if defined(NDEBUG)
#error "Flexwork cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Flexwork Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 1) return 140000 * COIN;
		
	int64_t nSubsidy;
	
	if( nHeight > 1 && nHeight <= 16800 ) {
	        nSubsidy = 1 * COIN;
	} else if( nHeight > 16800 && nHeight <= 168000 ) {
	        nSubsidy = 10 * COIN;
	} else {
		nSubsidy = 6 * COIN;
	}
...
{
    RenameThread("flexwork-scriptch");
